    ...
    foreach($serverIp as $server_ip){
        if($inbound_id == 0) {
            if($protocol == 'vless'){
                ...
                if($netType == 'grpc'){
                    if($tlsStatus == 'tls'){
                        $outputlink = "$protocol://$uniqid@$server_ip:$port?type=$netType&security=$tlsStatus&fp=firefox&alpn=h2%2Chttp%2F1.1&serviceName=$serviceName&sni=$serverName#$remark";
                    }else{
                        $outputlink = "$protocol://$uniqid@$server_ip:$port?type=$netType&security=$tlsStatus&fp=firefox&alpn=h2%2Chttp%2F1.1&serviceName=$serviceName#$remark";
                    }
            ...
            if($protocol == 'trojan'){
                ...
                $outputlink = "$protocol://$uniqid@$server_ip:$port?security=$tlsStatus{$psting}&fp=firefox&alpn=h2%2Chttp%2F1.1#$remark";
            ...
            if($protocol == 'vless'){
                ...
                if($netType == 'tcp'){
                    if($netType == 'tcp' and $header_type == 'http') $psting .= '&headerType=http';
                    if($tlsStatus=="xtls") $psting .= "&flow=xtls-rprx-direct&fp=firefox&alpn=h2%2Chttp%2F1.1";
                    if($tlsStatus=="reality") $psting .= "&fp=$fp&pbk=$pbk&sni=$sni" . ($flow != ""?"&flow=$flow":"") . "&sid=$sid&spx=$spiderX";
                    else $psting .= "&path=/&host=$host";
                    $outputlink = "$protocol://$uniqid@$server_ip:$port?type=$netType&security=$tlsStatus{$psting}#$remark";
                }elseif($netType == 'ws'){
                    if($rahgozar == true)$outputlink = "$protocol://$uniqid@$server_ip:443?type=$netType&security=tls&path=" . urlencode("$path?ed=2048") . "&encryption=none&host=$server_ip{$psting}&fp=firefox&alpn=h2%2Chttp%2F1.1#$remark";
                    else $outputlink = "$protocol://$uniqid@$server_ip:$port?type=$netType&security=$tlsStatus&path=/&host=$host{$psting}&fp=firefox&alpn=h2%2Chttp%2F1.1#$remark";
                }
                elseif($netType == 'kcp')
                    $outputlink = "$protocol://$uniqid@$server_ip:$port?type=$netType&security=$tlsStatus&fp=firefox&alpn=h2%2Chttp%2F1.1&headerType=$kcpType&seed=$kcpSeed#$remark";
                elseif($netType == 'grpc'){
                    if($tlsStatus == 'tls'){
                        $outputlink = "$protocol://$uniqid@$server_ip:$port?type=$netType&security=$tlsStatus&serviceName=$serviceName&sni=$serverName&fp=firefox&alpn=h2%2Chttp%2F1.1#$remark";
                    }else{
                        $outputlink = "$protocol://$uniqid@$server_ip:$port?type=$netType&security=$tlsStatus&serviceName=$serviceName&fp=firefox&alpn=h2%2Chttp%2F1.1#$remark";
                    }
                }
                ...
                $outputlink = "$protocol://$uniqid@$server_ip:$port{$psting}&fp=firefox&alpn=h2%2Chttp%2F1.1#$remark";
            }elseif($protocol == 'vmess'){
                $vmessArr = [
                    "v"=> "2",
                    "ps"=> $remark,
